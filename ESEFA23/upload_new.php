...
echo '<h1 class="page-head-line">Upload a new file to FileStorage</h1>';
...
if(isset($_REQUEST['err']) && ($_REQUEST['err'] == "error_ftype"))
{
	...
	$sql = "INSERT INTO `errors`
			...
			VALUES ('ERROR','File uploaded is not a pdf','Try again',STR_TO_DATE('$g_date', '%Y-%m-%d %H:%i:%s'),'upload_new')";
...
else if(isset($_REQUEST['err']) && ($_REQUEST['err'] == "error_id"))
{
	echo '<div class="alert alert-danger alert-dismissbale">';
	echo '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>';
	echo 'ERROR: Invalid Loan Number, please try again</div>';
	...
	$sql = "INSERT INTO `errors`
			(`e_status`,`e_msg`,`e_action`,`e_date`,`e_call`)
			VALUES ('ERROR','Invalid Loan Number','Try again',STR_TO_DATE('$g_date', '%Y-%m-%d %H:%i:%s'),'upload_new')";
	$result=$dblink->query($sql) or
			die("Something went wrong with: $sql<br>".$dblink->error);
}
else if(isset($_REQUEST['err']) && ($_REQUEST['err'] == "error_efile"))
{
	...
	$sql = "INSERT INTO `errors`
			(`e_status`,`e_msg`,`e_action`,`e_date`,`e_call`)
			VALUES ('ERROR','There was no file uploaded','Try again',STR_TO_DATE('$g_date', '%Y-%m-%d %H:%i:%s'),'upload_new')";
	$result=$dblink->query($sql) or
			die("Something went wrong with: $sql<br>".$dblink->error);
}
...
if(isset($_POST['submit']))
{
	...
	{
		redirect("https://ec2-3-15-170-107.us-east-2.compute.amazonaws.com/upload_new.php?err=error_id");
	}
	else
	{
		$fType=$_POST['ftype'];
		...
		{
			redirect("https://ec2-3-15-170-107.us-east-2.compute.amazonaws.com/upload_new.php?err=error_efile");
		}
		else if(mime_content_type($tmpName) != 'application/pdf')
		{
			redirect("https://ec2-3-15-170-107.us-east-2.compute.amazonaws.com/upload_new.php?err=error_ftype");
		}
		else
		{
			...
			$sql = "INSERT INTO `file_records_final`
					...
					VALUES ('$fileName','$f_size','$fType','$normalDate','$l_num','web_upload')";
			$result=$dblink->query($sql) or
				die("Something went wrong with: $sql<br>".$dblink->error);
			...
			$sql = "INSERT INTO `calls`
					...
					VALUES ('web_upload_new','$execution_time','$g_date','1')";
			$result=$dblink->query($sql) or
				die("Something went wrong with: $sql<br>".$dblink->error);
