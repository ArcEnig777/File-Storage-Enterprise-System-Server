...
<head>
<meta charset="utf-8">
<title>Search By File Date</title>
<!-- BOOTSTRAP STYLES-->
...
echo '<h1 class="page-head-line">Search By File Date</h1>';
...
else if(isset($_REQUEST['err']) && ($_REQUEST['err'] == "error_id"))
{
	echo '<div class="alert alert-danger alert-dismissbale">';
	echo '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>';
	echo 'ERROR: Invalid Date Format, please try again (ex: YYYY-MM-DD)</div>';
	...
	$sql = "INSERT INTO `errors`
			...
			VALUES ('ERROR','Invalid Date Format','Try again',STR_TO_DATE('$g_date', '%Y-%m-%d %H:%i:%s'),'search_file_date')";
...
else if(isset($_REQUEST['err']) && ($_REQUEST['err'] == "not_found"))
{
	echo '<div class="alert alert-danger alert-dismissbale">';
	echo '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>';
	echo 'ERROR: File Date not found, please try another File Date</div>';
	...
	$sql = "INSERT INTO `errors`
			(`e_status`,`e_msg`,`e_action`,`e_date`,`e_call`)
			VALUES ('ERROR','File Date not found','Try another File Date',STR_TO_DATE('$g_date', '%Y-%m-%d %H:%i:%s'),'search_file_date')";
	$result=$dblink->query($sql) or
			die("Something went wrong with: $sql<br>".$dblink->error);
}
$_REQUEST = array();
...
{
	echo '<form method="post" action="search_file_date.php">';
	echo '<div class="form-group">';
	echo '<label for="loanNum" class="control-label">File Date (FORMAT: YYYY-MM-DD)</label>';
	echo '<input type="text" name="fdate">';
	echo '</div>';
...
else if(isset($_POST['submit']) && $_POST['submit']=="submit")
{
	...
	if(!preg_match('/[0-9]{4}-[0-9]{2}-[0-9]{2}/', $fdate))
	{
		redirect("https://ec2-3-15-170-107.us-east-2.compute.amazonaws.com/search_file_date.php?err=error_id");
	}
	
	$time_start=microtime(true);
	
	$sql="SELECT `file_id`,`file_name`,`file_size`,`loan_number`,`date_created`,`file_type`
		  FROM `file_records_final`
		  WHERE `date_created` LIKE '$fdate%'";
	$result=$dblink->query($sql);
	if (strlen($dblink->error)>0)
	{
		$sql = "INSERT INTO `errors`
				(`e_status`,`e_msg`,`e_action`,`e_date`,`e_call`)
				VALUES ('ERROR','$dblink->error','Try again',STR_TO_DATE('$g_date', '%Y-%m-%d %H:%i:%s'),'search_file_date')";
		$result=$dblink->query($sql) or
				die("Something went wrong with: $sql<br>".$dblink->error);

		redirect("https://ec2-3-15-170-107.us-east-2.compute.amazonaws.com/search_file_date.php?err=error_query");
	}
	else if($result->num_rows<=0)
	{
		redirect("https://ec2-3-15-170-107.us-east-2.compute.amazonaws.com/search_file_date.php?err=no_files");
	}
	
	echo '<h3>Search results for File Date: '.$fdate.'</h3>';
	...
	$sql = "INSERT INTO `calls`
			...
			VALUES ('search_file_date','$execution_time','$g_date','1')";
	$result=$dblink->query($sql) or
		die("Something went wrong with: $sql<br>".$dblink->error);
